...
<div class="row">
            ...
            <div class="col-md-12">
                <div class="portlet box green">
                    <div class="portlet-title" style="border-bottom:none;">
                     ...
                     <button type="submit" class="btn green download"><?php echo $this->Html->link('Download', array( 'controller' => 'courses', 'action' => 'admin_courseListExport','?' => array('rail_id' => $_GET['rail_id'],'nonrail_id' => $_GET['nonrail_id']))); ?> </button>
                   <label>&nbsp;&nbsp;</label>
                        <!--end-->
                    </div>
        ...
        <button class="btn btn-success" type="button" name="send_archive" id="send_archive">Archive</button>                  
                </div>
            </div>
        </div>
...
$.ajax({
        ...
        type: "POST",
        evalScripts: true,
        url: '<?php echo Router::url(array('controller'=>'courses','action'=>'admin_archiveCourse'));?>',
        data: ({'courseId':test}),
        success: function (data){
            if(data=='1')
            {
              $("html, body").animate({ scrollTop: 0 }, "slow");
               document.getElementById("msgs_ajax").innerHTML = "You have successfully Archived the courses.";
             setTimeout(function(){  window.location.href = 'https://techno.sdssoftltd.co.uk/admin/courses/courseList';
 }, 5000);
            }
            else
            {
