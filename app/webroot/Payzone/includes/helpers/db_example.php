...
class DBDemo
{
  ...
  // const username = "technosd_techno";
  // const password = "Techno2018@";
   ...
   */
  public static function saveTransaction($user_id,$course_id,$order_id,$amounttidy,$amountminor,$amountmajor,$currency,$cross_reference,$status_code,$status,$transaction_datetime,$gateway_message,$integration)
  {
    ...
    $sqlCheck = "SELECT `id` FROM `sds_cta_transaction_logs` WHERE  `order_id` = '$order_id' AND `cross_reference` = '$cross_reference';";
    $sqlCheck = $mysqli->query($sqlCheck);
    if ($sqlCheck->num_rows==0){
      $sqlInsert = "INSERT INTO `sds_cta_transaction_logs` (`user_id`,`course_id`,`order_id`,`amount_`,`amount_minor`,`amount_major`,`currency`,`cross_reference`,`status_code`,`status`,`transaction_datetime_txt`,`transaction_datetime`,`gateway_message`,`integration_type`) VALUES ('$user_id','$course_id','$order_id','$amounttidy','$amountminor','$amountmajor','$currency','$cross_reference','$status_code','$status','$transaction_datetime','$timestamp','$gateway_message','$integration');";
